...
#region    ---   执行操作 ExternalEvent.Raise 与 IExternalEventHandler.Execute
		...
		private bool CheckUI()
		{
			...
			{
				if (Utils.String2Date(strDate, ref this.StartedDate)) // 说明不能直接转化为日期
				{
			...
			if (!string.IsNullOrEmpty(strDate))
			{
				if (Utils.String2Date(strDate, ref this.CompletedDate)) // 说明不能直接转化为日期
				{
	...
	private void DrawSoilFromCurve(CurveArrArray CurveArrArr)
	{
		...
		OldW.Soil.Soil_Model soil = default(OldW.Soil.Soil_Model);
		
		if (this.RadioBtn_ExcavSoil.Checked) // 绘制开挖土体
		{
			
			soil = this.ExcavDoc.FindSoilModel();
			OldW.Soil.Soil_Excav ex = this.ExcavDoc.CreateExcavationSoil(soil, this.Depth, CurveArrArr, DesiredName);
			
			// 设置开挖开始或者完成的时间
			if ((this.StartedDate != null) || (this.CompletedDate != null))
			{
				using (Transaction t = new Transaction(Document, "设置开挖开始或者完成的时间"))
				{
					t.Start();
