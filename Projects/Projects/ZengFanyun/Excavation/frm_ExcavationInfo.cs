...
public Request Id
		...
		private class ExcavSoilInfo
		{
			...
			[System.ComponentModel.Browsable(false)]public OldW.Soil.Soil_Excav Soil {get; set;}
...
#region    ---   Properties
		...
		private OldW.Soil.Soil_Model F_SoilModel;
private OldW.Soil.Soil_Model SoilModel
		{
			get
			{
				if (F_SoilModel != null)
...
#region    ---   执行操作 ExternalEvent.Raise 与 IExternalEventHandler.Execute
		...
		private BindingList<ExcavSoilInfo> GetExcavSoilInfo()
		{
			...
			if (V != null)
			{
				...
				foreach (OldW.Soil.Soil_Excav es in Ses)
				{
