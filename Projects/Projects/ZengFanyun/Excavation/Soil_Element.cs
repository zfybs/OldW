...
public FamilyInstance Soil
		...
		protected OldW.Excavation.ExcavationDoc F_ExcavDoc;
public OldW.Excavation.ExcavationDoc ExcavDoc
		{
			get
			{
				return F_ExcavDoc;
...
#region    ---   构造函数，通过 OldWDocument.GetSoilModel，或者是Create静态方法
		...
		protected Soil_Element(OldW.Excavation.ExcavationDoc ExcavDoc, FamilyInstance SoilElement)
		{
		...
		protected static bool IsSoildElement(Document doc, ElementId SoilElementId, ref string FailureMessage)
		{
			...
			if (SoilElementId != ElementId.InvalidElementId) // 说明用户手动指定了土体单元的ElementId，此时需要检测此指定的土体单元是否是有效的土体单元
			{
				try
				{
					...
					// 进行细致的检测
					//1. 族实例的族名称
					if (string.Compare(System.Convert.ToString(Soil.Symbol.FamilyName), GlobalSettings.Constants.FamilyName_Soil, true) != 0)
					{
						throw (new TypeUnloadedException(string.Format("指定的ElementId所对应的单元的族名称与全局的土体族的名称\"{0}\"不相同。", GlobalSettings.Constants.FamilyName_Soil)));
					}
					//2. 族实例的类别
