...
#region    ---   Fields
        ...
        public double? FindAdjacentEarthElevation(FamilyInstance SoilElement)
		{
			...
			// 将测斜管的底部端点作为原点
			double Length = System.Convert.ToDouble(Monitor.Symbol.LookupParameter("深度").AsDouble()); // 测斜管长度
			...
			ReferenceIntersector IntersectedEarth = new ReferenceIntersector(SoilElement.Id, FindReferenceTarget.Face, view3d);
		...
		private XYZ FindBottomPoint(XYZ ptInclinometerBottom, bool IsInside, FamilyInstance Earth, ReferenceIntersector IntersectedEarth)
		{
			...
			else
			{
				...
				for (var angle =( 0.3 / 180 * Math.PI); angle <= 2 * Math.PI; angle += Math.PI / 7)
				{
					...
					dire = new XYZ(System.Math.Cos(angle), System.Math.Sin(angle), 0);
					...
					{
						NearestDir = dire;
						NearestDist = System.Convert.ToDouble(refContext.Proximity);
				...
				if (NearestRefCont != null)
				{
					...
					// 测斜管底部点到找到的最近的面的垂足点
					XYZ Normal = VerticalFace.ComputeNormal(NearestRef.UVPoint); // face normal where ray hits，注意：这里返回的法向量是相对于族类型的局部空间的
