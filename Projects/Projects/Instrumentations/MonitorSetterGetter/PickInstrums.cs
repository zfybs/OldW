...
namespace OldW.Instrumentations.MonitorSetterGetter
{
    ...
    public partial class PickInstrums : Form
    {
        ...
        private void ConstructTreeView(ICollection<Instrumentation> eleIdCollection)
        {
            ...
            {
                string instrumTypeName = Enum.GetName(typeof (InstrumentationType), ins.Type); // 要放置的测点的名称 
                if (!insType.Contains(ins.Type))
                {
                    // 创建一个新的测点类型节点，并设置属性
                    TreeNode ndType = new TreeNode();

                    ndType.Name = instrumTypeName;  // 要放置的测点的名称 ins.Type.ToString();
                    ndType.Tag = ins.Type;
                ...
                TreeViewIns.Nodes[instrumTypeName].Nodes.Add(ndIns);
            }
        }
