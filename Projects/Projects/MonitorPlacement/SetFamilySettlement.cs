        ...
        public class cmd_SetFamilySettlement : IExternalCommand
        {
            ...
            public Result Execute(ExternalCommandData commandData, ref string message, ElementSet elements)
            {
                try
                {
                    ...
                    //是否存在族
                    Boolean found = FilterTools.existFamliyByName(uidoc.Document, InstrumentationType.地表隆沉.ToString(), out foundId);
                    ...
                    else
                    {
                        ...
                        trans.Start();
                        uidoc.Document.LoadFamily(Path.Combine(GlobalSettings.ProjectPath.Path_family, 
                            InstrumentationType.地表隆沉.ToString() + ".rfa"), out family);
                        trans.Commit();
